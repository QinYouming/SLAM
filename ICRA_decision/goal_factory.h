...
//    memcpy(charmap_, costmap_ptr_->GetCostMap()->GetCharMap(), costmap_2d_->GetSizeXCell() * costmap_2d_->GetSizeYCell());
  ...
  ~GoalFactory() = default;
	...
	void PoseGoal(int index){
		...
		*/
		        geometry_msgs::PoseStamped pose_goal_ = POI_goals_[index];
       			printf("POSE GOAL %f %f \n", pose_goal_.pose.position.x, pose_goal_.pose.position.y);
...
//          costmap_2d_->GetCost(goal_cell_x, goal_cell_y);
          ...
          if (obstacle_count > 5) { //TOD:  this should write in the proto file
            break;
          }
        }
...
/*
/// //--------------------modified by zitong ---------------//
	POI_goals_.resize(decision_config.point().size());
	for (int i = 0; i < decision_config.point().size(); i ++){
	      POI_goals_[i].header.frame_id = "map";
	      POI_goals_[i].pose.position.x = decision_config.point(i).x();
	      POI_goals_[i].pose.position.y = decision_config.point(i).y();
	      POI_goals_[i].pose.position.z = decision_config.point(i).z();
	      ...
	      POI_goals_[i].pose.orientation.x = quaternion.x();
	      POI_goals_[i].pose.orientation.y = quaternion.y();
	      POI_goals_[i].pose.orientation.z = quaternion.z();
	      POI_goals_[i].pose.orientation.w = quaternion.w();
    }
//-------------------------------------------------------
 */
...
//      lost_enemy_ = true;
 ...
 protected:
//----------------modified by zitong --------------------//
	std::vector<geometry_msgs::PoseStamped> POI_goals_;
//------------------------------------------------------//
